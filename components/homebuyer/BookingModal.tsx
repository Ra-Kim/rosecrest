...
import {
  X, MapPin, Home, Bed, Clock, Zap, CheckCircle, Circle, PoundSterling,
} from "lucide-react";
...
const BookingModal = ({ isOpen, onClose, surveyLevel = 2 }: BookingModalProps) => {
  ...
  const {
    ...
    defaultValues: {
      ...
      jobAddress: "", jobTown: "", jobPostcode: "",
      propertyType: "", propertyValue: "", bedrooms: "",
      surveyType: defaultSurveyType, purchaseStage: "",
      timeline: "", helpWith: "", expressService: false,
    },
  });

  const expressServiceChecked = useWatch({ control, name: "expressService", defaultValue: false });
  const selectedSurveyType = useWatch({ control, name: "surveyType", defaultValue: defaultSurveyType });
    ...
    >
      <div className="relative w-full max-w-5xl max-h-[92vh] flex flex-col bg-white rounded-4xl shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-8 pt-7 pb-5 shrink-0">
          <h2 className="text-xl font-bold text-[#101828]">Check Availability & Get a Fixed Price</h2>
        ...
        <div className="overflow-y-auto px-8 pb-2 flex-1">
          {isSubmitSuccessful ? (
            <div className="flex flex-col items-center text-center py-16 gap-4">
              ...
              <h3 className="text-xl font-bold text-[#101828]">Booking Request Received</h3>
              <p className={`${sourceSans.className} text-[#4A5565] text-sm max-w-xs`}>
                Thank you. A member of our team will be in touch to confirm your inspection date.
              </p>
              <Button onClick={onClose} className="mt-4 rounded-full px-8 bg-[#262A6F] text-white">Close</Button>
            </div>
          ) : (
            <div className="grid lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-5">
                <div className="bg-[#EFF6FF] rounded-xl p-3.5 flex items-start gap-3">
                  ...
                  <p className={`text-sm text-[#364153] ${sourceSans.className}`}>
                    Provide a few details and we&apos;ll instantly show your survey price. Takes less than 60 seconds.
                  </p>
                </div>
                ...
                <form id="booking-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input className="h-11 rounded-xl border-[#E5E7EB] text-sm" {...register("firstName")} />
                      {errors.firstName && <p className="text-xs text-red-500">{errors.firstName.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium text-gray-700">Last Name</Label>
                      <Input className="h-11 rounded-xl border-[#E5E7EB] text-sm" {...register("lastName")} />
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input type="email" className="h-11 rounded-xl border-[#E5E7EB] text-sm" {...register("email")} />
                      {errors.email && <p className="text-xs text-red-500">{errors.email.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium text-gray-700">Phone</Label>
                      <Input type="tel" className="h-11 rounded-xl border-[#E5E7EB] text-sm" {...register("phone")} />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="12 Example Street" className="pl-9 h-11 rounded-xl border-[#E5E7EB] text-sm" {...register("jobAddress")} />
                    </div>
                    {errors.jobAddress && <p className="text-xs text-red-500">{errors.jobAddress.message}</p>}
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium text-gray-700">Town / City</Label>
                      <Input placeholder="London" className="h-11 rounded-xl border-[#E5E7EB] text-sm" {...register("jobTown")} />
                      {errors.jobTown && <p className="text-xs text-red-500">{errors.jobTown.message}</p>}
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-sm font-medium text-gray-700">Postcode</Label>
                      <Input placeholder="SW1A 1AA" className="h-11 rounded-xl border-[#E5E7EB] text-sm" {...register("jobPostcode")} />
                      {errors.jobPostcode && <p className="text-xs text-red-500">{errors.jobPostcode.message}</p>}
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="e.g. 350000" type="number" min="0" className="pl-9 h-11 rounded-xl border-[#E5E7EB] text-sm" {...register("propertyValue")} />
                    </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Controller name="surveyType" control={control} render={({ field }) => (
                      <Select key={defaultSurveyType} onValueChange={field.onChange} defaultValue={defaultSurveyType}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Controller name="purchaseStage" control={control} render={({ field }) => (
                      <Select onValueChange={field.onChange} value={field.value}>
                        <SelectTrigger className="h-11 rounded-xl border-[#E5E7EB] w-full text-sm">
                          <SelectValue placeholder="Select stage" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="Just viewing">Just viewing</SelectItem>
                          <SelectItem value="Offer made">Offer made</SelectItem>
                          <SelectItem value="Offer accepted">Offer accepted</SelectItem>
                          <SelectItem value="Exchanged contracts">Exchanged contracts</SelectItem>
                        </SelectContent>
                      </Select>
                    )} />
                  ...
                  <div className="bg-amber-50 rounded-xl p-3.5 border border-amber-200">
                    <Controller name="expressService" control={control} render={({ field }) => (
              ...
              <div className="border border-[#E5E7EB] rounded-2xl p-5 h-fit sticky top-0">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className={`text-xs font-medium text-[#6A7282] ${sourceSans.className}`}>Quote preview</span>
                </div>
                <h3 className="text-base font-bold text-[#101828] mb-5">{activeSurveyTitle}</h3>
                ...
                <div className="bg-[#EFF6FF] rounded-xl p-4 mt-5">
                  ...
                  <ul className={`space-y-2.5 ${sourceSans.className}`}>
                    {["Confirm your booking", "We'll contact you to arrange the inspection", "Receive your detailed report"].map((step, i) => (
                      <li key={i} className="flex items-start gap-3">
                        <div className="bg-[#262A6F] text-white rounded-full w-5 h-5 flex items-center justify-center shrink-0 text-xs font-semibold">{i + 1}</div>
        ...
        {!isSubmitSuccessful && (
          <div className="flex items-center justify-between px-8 py-5 border-t border-[#F3F4F6] shrink-0 bg-white">
            <button type="button" onClick={onClose} className={`text-sm text-[#4A5565] hover:text-[#101828] transition-colors ${sourceSans.className}`}>
              Cancel
            </button>
            <Button
              type="submit"
