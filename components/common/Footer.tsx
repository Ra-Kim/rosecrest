...
const Footer: React.FC<FooterProps> = ({ quickLinks }) => {
  ...
  const resolvedLinks = quickLinks ?? defaultQuickLinks;
  ...
  const contactInfo = [
    ...
    {
      icon: MapPin,
      label: "71-75 Shelton Street, Covent Garden, London, United Kingdom, WC2H 9JQ",
      href: "#",
    },
  ];

  const accreditations = ["RICS Regulated", "Constructionline Gold", "SafeContractor"];
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8 py-12 lg:py-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12 lg:gap-16">
            ...
            <div className="space-y-6">
              ...
              <p className={`${sourceSans.className} text-[#64748B] text-sm lg:text-base leading-relaxed max-w-sm mt-6`}>
                Professional property inspection and reporting services across the UK. RICS regulated
                  ...
                  <li key={index}>
                    {link.onClick ? (
                      <button
                        ...
                        className={`${sourceSans.className} text-[#64748B] hover:text-[#1E3A8A] transition-colors text-sm lg:text-base text-left`}
                      >
                        {link.label}
                      </button>
                    ) : (
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-end gap-4 lg:gap-6">
                {accreditations.map((accreditation, index) => (
                  <React.Fragment key={index}>
                    <span className={`${sourceSans.className} text-[#64748B] text-xs lg:text-sm`}>
                      {accreditation}
                    </span>
