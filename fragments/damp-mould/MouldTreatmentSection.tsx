...
const RATE_MID  = 25;
const RATE_LOW  = Math.round(RATE_MID * 0.9);
const RATE_HIGH = Math.round(RATE_MID * 1.1);
...
const MouldTreatmentSection = () => {
  ...
  const mid  = hasValue ? Math.round(((low ?? 0) + (high ?? 0)) / 2) : null;

  // Build context string that pre-fills the modal with area size + estimate
  const enquiryContext = hasValue
    ? `Damp and Mould Treatment — ${parsed}m² (estimate £${low}–£${high})`
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        ...
        <div className="flex flex-col">
          <Reveal animation="fade-right" duration={600}>
            ...
            <p className={`${sourceSans.className} text-[#4A5565] text-base lg:text-lg mb-8`}>
          ...
          <Reveal animation="fade-up" duration={500} delay={150}>
            <div className="border border-[#E5E7EB] rounded-2xl p-6 mb-8 space-y-4">
              <div className="space-y-2">
                <Label className={`${sourceSans.className} text-sm text-[#4A5565]`}>
              ...
              {hasValue ? (
                <div className="bg-[#F0F3FF] border border-[#D0D7F5] rounded-xl p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className={`${sourceSans.className} text-xs text-[#6A7282] mt-1`}>
                  ...
                  <div className="mt-4 pt-4 border-t border-[#D0D7F5] grid grid-cols-3 gap-3 text-center">
                    <div>
                      <p className={`${sourceSans.className} text-xs text-[#6A7282]`}>Low estimate</p>
                      <p className="text-sm font-bold text-[#262A6F]">£{low}</p>
                    </div>
                    <div className="border-x border-[#D0D7F5]">
                      <p className={`${sourceSans.className} text-xs text-[#6A7282]`}>Mid estimate</p>
                      <p className="text-sm font-bold text-[#262A6F]">£{mid}</p>
                    </div>
                    <div>
                      <p className={`${sourceSans.className} text-xs text-[#6A7282]`}>High estimate</p>
                      <p className="text-sm font-bold text-[#262A6F]">£{high}</p>
                    </div>
                  </div>

                  <p className={`${sourceSans.className} text-xs text-[#9CA3AF] mt-3`}>
                ...
                <div className="border border-dashed border-[#E5E7EB] rounded-xl p-6 flex flex-col items-center justify-center text-center gap-6 min-h-43.5">
                  ...
                  <p className={`${sourceSans.className} text-sm text-[#9CA3AF] max-w-xs`}>
                    Enter the affected area above and the estimated treatment cost will appear here.
                  </p>
                </div>
              )}
            </div>
          ...
          <Reveal animation="fade-up" duration={400} delay={100}>
            <Button
              onClick={() => openEnquiry(enquiryContext)}
              className="bg-[#262A6F] hover:bg-[#262A6F]/90 text-white rounded-full px-8 py-6 text-base font-semibold inline-flex items-center gap-2 w-fit"
            >
              Book a Discussion
