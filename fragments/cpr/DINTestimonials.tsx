...
const TestimonialCard = ({ t }: { t: (typeof testimonials)[0] }) => (
  <div className="bg-white border border-gray-200 rounded-4xl p-8 flex flex-col gap-6 h-full">
    ...
    <p className={`${sourceSans.className} text-[#101828] font-light text-base md:text-lg leading-relaxed flex-1`}>
      {t.quote}
    </p>
    <div className="flex items-center gap-3">
      <div className="w-12 h-12 rounded-full overflow-hidden bg-white border border-gray-200 shrink-0 flex items-center justify-center">
        <Image src={t.avatar} alt={t.org} width={48} height={48} className="object-contain w-full h-full" />
      </div>
      <div>
...
const DINTestimonials = () => {
  ...
  useEffect(() => {
    if (mobileTotal <= 1 && desktopTotal <= 1) return;
        ...
        <Reveal animation="fade-up" duration={600} delay={100}>
          <div className="text-center mb-12">
            <h2 className="text-4xl lg:text-5xl font-bold text-[#101828] mb-4">
              Testimonials from DIN Solicitors
            </h2>
            <p className={`${sourceSans.className} text-[#4A5565] text-base lg:text-lg mx-auto`}>
              Here&apos;s what our DIN Solicitors are saying
            </p>
          </div>
        </Reveal>
          ...
          {mobileTotal > 1 && (
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={prevMobile} className="w-10 h-10 rounded-[6px] bg-[#262A6F] hover:bg-[#262A6F]/80 text-white flex items-center justify-center transition-colors" aria-label="Previous">
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button onClick={nextMobile} className="w-10 h-10 rounded-[6px] bg-[#262A6F] hover:bg-[#262A6F]/80 text-white flex items-center justify-center transition-colors" aria-label="Next">
        ...
        <div className="hidden lg:block">
          ...
          {desktopTotal > 1 && (
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={prevDesktop} className="w-10 h-10 rounded-[6px] bg-[#262A6F] hover:bg-[#262A6F]/80 text-white flex items-center justify-center transition-colors" aria-label="Previous">
                  <ChevronLeft className="w-5 h-5" />
                </button>
                <button onClick={nextDesktop} className="w-10 h-10 rounded-[6px] bg-[#262A6F] hover:bg-[#262A6F]/80 text-white flex items-center justify-center transition-colors" aria-label="Next">
                  <ChevronRight className="w-5 h-5" />
                </button>
              </div>
            </div>
...
export default DINTestimonials;