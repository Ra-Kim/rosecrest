...
import {
  MapPin, Home, Bed, Clock, Zap, CheckCircle, Circle, PoundSterling,
} from "lucide-react";
...
const BookingForm = ({ surveyLevel, surveyTitle, basePrice }: BookingFormProps) => {
  ...
  const { register, handleSubmit, control, formState: { errors } } = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      surveyType, purchaseStage: "", timeline: "", helpWith: "",
      expressService: false,
    },
  });

  const expressServiceChecked = useWatch({ control, name: "expressService", defaultValue: false });
  const totalPrice = basePrice + (expressServiceChecked ? expressServiceCost : 0);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-[24px] p-6 lg:p-8 border border-[#E5E7EB] space-y-5">
            ...
            <form id="booking-form-inline" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="bg-amber-50 rounded-xl p-3.5 border border-amber-200">
                <Controller name="expressService" control={control} render={({ field }) => (
                  <div className="flex items-start gap-3">
                    ...
                    />
                    <Label htmlFor="expressService-inline" className="flex items-start gap-2 cursor-pointer">
          ...
          <div className="bg-white rounded-[24px] p-6 lg:p-8 border border-[#E5E7EB]">
            <div className="p-6 lg:p-8 rounded-3xl border border-[#E5E7EB]">
              <div className="flex items-center gap-2 mb-4">
                ...
                <span className={`${sourceSans.className} text-sm font-medium text-[#6A7282]`}>Quote preview</span>
              </div>
              <h3 className="text-lg font-bold text-[#101828] mb-6">{surveyTitle}</h3>
              <div className={`space-y-4 mb-6 ${sourceSans.className}`}>
                ...
                <div className="pt-4 border-t border-gray-200">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-[#101828]">Total (incl. VAT)</span>
                    <span className="text-2xl font-bold text-[#101828]">£{totalPrice}</span>
                  </div>
                </div>
              </div>
              <div className="bg-[#EFF6FF] rounded-[10px] p-4">
                <h4 className="font-bold text-[#101828] mb-4 text-sm">What happens next</h4>
                <ul className={`space-y-3 text-sm ${sourceSans.className}`}>
                  {["Confirm your booking", "We'll contact you to arrange the inspection", "Receive your detailed report"].map((step, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <div className="bg-[#262A6F] text-white rounded-full w-5 h-5 flex items-center justify-center shrink-0 text-sm font-semibold">{i + 1}</div>
